...
def fetch_pdf_url():
    ...
    for u in pdfs:
        try:
            ...
            if rr.ok and ("pdf" in ctype or rr.content[:4] == b"%PDF"):
...
def parse_week_range(text):
    ...
    m = re.search(r"(\d{1,2})\.(\d{1,2})\.?\s*[-–]\s*(\d{1,2})\.(\d{1,2})\.?(?:\s*(\d{4}))?", compact)
    if not m:
...
def validate_current_week(start, end):
    ...
    if start != monday or end != friday:
        raise RuntimeError(f"PDF nie je pre aktuálny pracovný týždeň: {start}–{end}, očakávam {monday}–{friday}")
